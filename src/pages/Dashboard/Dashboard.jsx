...
const Dashboard = () => {
  ...
  const { summary, conditionData, intervensiData, allKecamatan } = useMemo(() => {
    ...
    const calculateSummary = () => {
        const countSchools = (dataObj) => Object.values(dataObj).reduce((total, kecamatan) => total + (Array.isArray(kecamatan) ? kecamatan.length : 0), 0);
        const countTeachers = (dataObj) => Object.values(dataObj).reduce((total, kecamatan) => {
            if (!Array.isArray(kecamatan)) return total;
            return total + kecamatan.reduce((subTotal, school) => subTotal + (Number(school.teacher?.n_teachers) || Number(school.teachers) || 0), 0);
        }, 0);
        return {
    ...
    const calculateConditionData = () => {
        const result = { PAUD: { total: 0, baik: 0, sedang: 0, berat: 0, kurangRkb: 0 }, SD: { total: 0, baik: 0, sedang: 0, berat: 0, kurangRkb: 0 }, SMP: { total: 0, baik: 0, sedang: 0, berat: 0, kurangRkb: 0 }, PKBM: { total: 0, baik: 0, sedang: 0, berat: 0, kurangRkb: 0 } };
    ...
    const calculateIntervensiData = () => {
        const result = { PAUD: { rehab: 0, pembangunan: 0 }, SD: { rehab: 0, pembangunan: 0 }, SMP: { rehab: 0, pembangunan: 0 }, PKBM: { rehab: 0, pembangunan: 0 } };
    ...
    const getTopKecamatanPerJenjang = (dataObj, jenjangName) => {
        ...
        Object.entries(dataObj).forEach(([kecamatanName, schools]) => {
            ...
            schools.forEach(school => {
                ...
                if (condition) {
                    let value = 0;
                    if (kecamatanFilters.tipe === 'berat') value = Number(condition.classrooms_heavy_damage) || 0;
                    else if (kecamatanFilters.tipe === 'sedang') value = Number(condition.classrooms_moderate_damage) || 0;
                    else if (kecamatanFilters.tipe === 'kurangRkb') value = Number(condition.lacking_rkb) || 0;
                    totalValue += value;
                }
            });
        ...
        if (kecamatanFilters.urutan === 'teratas') kecamatanData.sort((a, b) => b.value - a.value);
        else kecamatanData.sort((a, b) => a.value - b.value);
        
        return kecamatanData.slice(0, 5).map(item => ({ name: `${item.kecamatanName} (${jenjangName})`, PAUD: jenjangName === 'PAUD' ? item.value : 0, SD: jenjangName === 'SD' ? item.value : 0, SMP: jenjangName === 'SMP' ? item.value : 0, PKBM: jenjangName === 'PKBM' ? item.value : 0, sortValue: item.value, jenjang: jenjangName, kecamatan: item.kecamatanName }));
    };
  ...
  const topDesaData = useMemo(() => {
    ...
    const processData = (dataObj, jenjangName) => {
        ...
        Object.entries(dataObj).forEach(([kecamatanName, schools]) => {
            if (desaFilters.kecamatan !== 'Semua Kecamatan' && desaFilters.kecamatan !== kecamatanName) return;
            if (!Array.isArray(schools)) return;
            schools.forEach(school => {
                ...
                if (desaName && condition) {
                    ...
                    if (!desaStats[key]) desaStats[key] = { name: desaName, kecamatan: kecamatanName, displayName: key, value: 0 };
                    let value = 0;
                    if (desaFilters.tipe === 'berat') value = Number(condition.classrooms_heavy_damage) || 0;
                    else if (desaFilters.tipe === 'sedang') value = Number(condition.classrooms_moderate_damage) || 0;
                    else if (desaFilters.tipe === 'kurangRkb') value = Number(condition.lacking_rkb) || 0;
  ...
  const conditionChartData = useMemo(() => {
    ...
    return [
        { jenjang: 'PAUD', 'Total Kelas': conditionData.PAUD.total, 'Kondisi Baik': conditionData.PAUD.baik, 'Rusak Sedang': conditionData.PAUD.sedang, 'Rusak Berat': conditionData.PAUD.berat, 'Kurang RKB': conditionData.PAUD.kurangRkb },
        { jenjang: 'SD', 'Total Kelas': conditionData.SD.total, 'Kondisi Baik': conditionData.SD.baik, 'Rusak Sedang': conditionData.SD.sedang, 'Rusak Berat': conditionData.SD.berat, 'Kurang RKB': conditionData.SD.kurangRkb },
        { jenjang: 'SMP', 'Total Kelas': conditionData.SMP.total, 'Kondisi Baik': conditionData.SMP.baik, 'Rusak Sedang': conditionData.SMP.sedang, 'Rusak Berat': conditionData.SMP.berat, 'Kurang RKB': conditionData.SMP.kurangRkb },
        { jenjang: 'PKBM', 'Total Kelas': conditionData.PKBM.total, 'Kondisi Baik': conditionData.PKBM.baik, 'Rusak Sedang': conditionData.PKBM.sedang, 'Rusak Berat': conditionData.PKBM.berat, 'Kurang RKB': conditionData.PKBM.kurangRkb }
    ];
  }, [conditionData]);
  ...
  const CustomLabel = (props) => {
    ...
    return (<text x={labelX} y={labelY} fill="#374151" fontSize="11" fontWeight="600" textAnchor="start" dominantBaseline="middle" style={{ filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.1))' }}>{value.toLocaleString()}</text>);
  };

  const CustomBar = ({ fill, dataKey, ...props }) => (<Bar {...props} dataKey={dataKey} fill={fill} radius={[0, 4, 4, 0]} minPointSize={20}><LabelList content={CustomLabel} /></Bar>);
    ...
    <div className={styles.dashboard}>
      ...
      <div className={styles.chartsContainer}>
        <div className={styles.chartCard}>
          ...
          <div className={styles.chartContent}>
            <ResponsiveContainer width="100%" height={350}>
              <BarChart data={conditionChartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                <CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="jenjang" /><YAxis /><Tooltip /><Legend />
        ...
        <div className={styles.chartCard}>
          ...
          <div className={styles.chartContent}>
            <ResponsiveContainer width="100%" height={350}>
              <BarChart data={intervensiChartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                <CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="jenjang" /><YAxis /><Tooltip /><Legend />
      ...
      <div className={styles.chartContainer}>
        <div className={styles.chartHeader}>
          ...
          <div className={styles.filtersContainer}>
            <div className={styles.filterGroup}><label>Tampilkan Grafik:</label><select value={kecamatanFilters.tipe} onChange={(e) => setKecamatanFilters({...kecamatanFilters, tipe: e.target.value})}><option value="berat">Rusak Berat</option><option value="sedang">Rusak Sedang</option><option value="kurangRkb">Kurang RKB</option></select></div>
            <div className={styles.filterGroup}><label>Urutan:</label><select value={kecamatanFilters.urutan} onChange={(e) => setKecamatanFilters({...kecamatanFilters, urutan: e.target.value})}><option value="teratas">Teratas</option><option value="terbawah">Terbawah</option></select></div>
        ...
        <div className={styles.chartOverflow}>
          <ResponsiveContainer width="100%" height={Math.max(600, topKecamatanData.length * 35)}>
            <BarChart data={topKecamatanData} layout="vertical" margin={{ top: 20, right: 100, left: 60, bottom: 20 }} barSize={25} barGap={4}>
              <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} />
              <XAxis type="number" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#6B7280' }} domain={[0, 'dataMax + 10']} />
              <YAxis type="category" dataKey="name" width={200} fontSize={10} interval={0} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#374151', textAnchor: 'end', dominantBaseline: 'middle' }} tickFormatter={(value) => (window.innerWidth < 768 && value.length > 25 ? value.substring(0, 22) + '...' : value)} />
              <Tooltip formatter={(value, name) => [value?.toLocaleString() || 0, name]} labelFormatter={(label) => `Kecamatan: ${label}`} contentStyle={{ backgroundColor: 'white', border: '1px solid #E5E7EB', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.05)' }} />
              <Legend wrapperStyle={{ paddingTop: '20px', fontSize: '12px' }} />
              <CustomBar dataKey="PAUD" fill="#EC4899" /><CustomBar dataKey="SD" fill="#DC2626" /><CustomBar dataKey="SMP" fill="#2563EB" /><CustomBar dataKey="PKBM" fill="#16A34A" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className={styles.chartContainer}>
        <div className={styles.chartHeader}>
          ...
          <div className={styles.filtersContainer}>
            <div className={styles.filterGroup}><label>Jenjang:</label><select value={desaFilters.jenjang} onChange={(e) => setDesaFilters({...desaFilters, jenjang: e.target.value})}><option value="Semua Jenjang">Semua Jenjang</option><option value="PAUD">PAUD</option><option value="SD">SD</option><option value="SMP">SMP</option><option value="PKBM">PKBM</option></select></div>
            <div className={styles.filterGroup}><label>Kecamatan:</label><select value={desaFilters.kecamatan} onChange={(e) => setDesaFilters({...desaFilters, kecamatan: e.target.value})}><option value="Semua Kecamatan">Semua Kecamatan</option>{allKecamatan.map(kec => (<option key={kec} value={kec}>{kec}</option>))}</select></div>
            <div className={styles.filterGroup}><label>Tipe:</label><select value={desaFilters.tipe} onChange={(e) => setDesaFilters({...desaFilters, tipe: e.target.value})}><option value="berat">Rusak Berat</option><option value="sedang">Rusak Sedang</option><option value="kurangRkb">Kurang RKB</option></select></div>
            <div className={styles.filterGroup}><label>Jumlah:</label><input type="number" min="1" max="50" value={desaFilters.jumlah} onChange={(e) => setDesaFilters({...desaFilters, jumlah: parseInt(e.target.value) || 20})} /></div>
            <div className={styles.filterGroup}><label>Urutan:</label><select value={desaFilters.urutan} onChange={(e) => setDesaFilters({...desaFilters, urutan: e.target.value})}><option value="teratas">Teratas</option><option value="terbawah">Terbawah</option></select></div>
          </div>
        </div>
        <ResponsiveContainer width="100%" height={Math.max(500, topDesaData.length * 30)}>
          <BarChart data={topDesaData} layout="vertical" margin={{ top: 20, right: 60, left: 50, bottom: 20 }} barSize={20}>
            <CartesianGrid strokeDasharray="3 3" /><XAxis type="number" /><YAxis type="category" dataKey="displayName" width={180} fontSize={10} interval={0} tick={{ fill: '#555', fontSize: 10 }} />
            <Tooltip />
            <Bar dataKey="value" fill="#4ecdc4"><LabelList content={CustomLabel} /></Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
