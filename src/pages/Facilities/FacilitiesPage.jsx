...
// Import komponen detail per jenjang (dipakai jika ingin render detail lokal)
...
///////////////////////////////////////////////////////////////
// === DataTable (disalin dari SchoolDetailPage.jsx, tanpa ubahan struktur)
///////////////////////////////////////////////////////////////
...
const FacilitiesPage = () => {
  ...
  // Custom label/tooltip untuk Pie & Bar
  ...
  const generateChartData = (data, allSchoolData, allKegiatanData) => {
    ...
    const totalPembangunan = rekap.sekolah_tanpa_toilet || 1; // hindari bagi nol
    setPembangunanPieData([
      { name: 'Kebutuhan Toilet (Belum dibangun)', value: Math.max(0, kebutuhan_belum_dibangun), percent: (Math.max(0, kebutuhan_belum_dibangun)/totalPembangunan)*100, color: '#FF6B6B' },
  ...
  // === onDetailClick: BUKA TAB/JENDELA BARU KE DETAIL TANPA LAYOUT ===
  const handleDetailClickNavigate = useCallback((row) => {
    const npsn = row?.npsn;
    if (!npsn) { alert('NPSN sekolah tidak ditemukan.'); return; }
    // Pakai URL absolut supaya aman kalau base berubah (Vite base, subpath, dsb)
    const url = new URL(`/detail-sekolah?from=facilities&npsn=${encodeURIComponent(npsn)}`, window.location.origin);
                  ...
                  <PieChart>
                    ...
                    >
                      {kondisiPieData.map((entry, index) => (
                        <Cell key={`cell-kond-${index}`} fill={entry.color} />
                      ))}
                    </Pie>
                  ...
                  <PieChart>
                    ...
                    >
                      {rehabilitasiPieData.map((entry, index) => (
                        <Cell key={`cell-rehab-${index}`} fill={entry.color} />
                      ))}
                    </Pie>
                    <Tooltip content={customPieTooltip} />
                    <Legend />
                  ...
                  <PieChart>
                    ...
                    >
                      {pembangunanPieData.map((entry, index) => (
                        <Cell key={`cell-bangun-${index}`} fill={entry.color} />
                      ))}
                    </Pie>
                    <Tooltip content={customPieTooltip} />
                    <Legend />
                  ...
                  <BarChart data={kondisiToiletData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                      ...
                      {kondisiToiletData.map((entry, index) => (
                        <Cell key={`cell-bar-kond-${index}`} fill={entry.color} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                  ...
                  <BarChart data={intervensiToiletData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                      ...
                      {intervensiToiletData.map((entry, index) => (
                        <Cell key={`cell-bar-int-${index}`} fill={entry.color} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
        ...
        <section className={`${styles.card} ${styles.tableCard}`}>
          ...
          <div className={styles.chartContent}>
            <DataTable
              ...
              onDetailClick={handleDetailClickNavigate}  // <<< NAVIGASI: buka tab baru detail (tanpa layout)
            />
          </div>
        </section>
      </div>
  ...
  return (
    <main className={styles.pageWrapper}>
      ...
      {/* Opsi render detail lokal (tidak dipakai jika selalu buka tab baru) */}
