// src/components/schools/SchoolDetail/Smp/SchoolDetailSmp.jsx - KODE FINAL DAN TERKOREKSI UNTUK SEMUA SINTAKS DAN LOGIKA DATA
...
const getData = (data, path, defaultValue = 0) => {
    ...
    // Memastikan 0.0 dari JSON dikembalikan sebagai 0, dan tidak ada null/undefined
...
const SchoolDetailSmp = ({ schoolData, onBack }) => {
  ...
  const handleLocationClick = () => {
    if (schoolData && schoolData.coordinates && schoolData.coordinates.length === 2) {
  ...
  // === DATA UNTUK BAR CHART KONDISI KELAS (NEED & INTERVENSI) ===
  ...
  // Nilai rehabilitasi diatur menjadi 1 sesuai permintaan, karena tidak ada di data JSON
  const rehabKegiatan = 1; 
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <h1 className={styles.schoolName}>{getData(schoolData, ['name'], 'Nama Sekolah Tidak Tersedia')}</h1>
        <div className={styles.schoolInfo}>
          <div className={styles.infoRow}><span className={styles.label}>NPSN</span><span className={styles.colon}>:</span><span className={styles.value}>{getData(schoolData, ['npsn'], '-')}</span></div>
          <div className={styles.infoRow}><span className={styles.label}>Alamat</span><span className={styles.colon}>:</span><span className={styles.value}>{getData(schoolData, ['address'], '-')}</span></div>
          <div className={styles.infoRow}><span className={styles.label}>Desa</span><span className={styles.colon}>:</span><span className={styles.value}>{getData(schoolData, ['village'], '-')}</span></div>
          ...
          <div className={styles.infoRow}><span className={styles.label}>Jumlah Siswa</span><span className={styles.colon}>:</span><span className={styles.value}>{getData(schoolData, ['student_count'], '0')}</span></div>
        </div>
        <button onClick={handleLocationClick} className={styles.locationButton}>
          Lihat Lokasi {getData(schoolData, ['name'], '')} di Google Maps ({schoolData.kecamatan || ''})
        </button>
      </div>

      {/* Kondisi & Intervensi Ruang Kelas Section (BAR CHART & DATA DETAIL) */}
      <div className={styles.section}>
        ...
        <div className={styles.card}>
          ...
          <div className={styles.dataRow}><span>Kondisi Kelas Baik (Total Ruangan Bersih): {getData(schoolData, ['class_condition', 'classrooms_good'], 0)}</span></div>
          <div className={styles.dataRow}><span>Kondisi Kelas Rusak Sedang: {rusakSedang}</span></div>
          <div className={styles.dataRow}><span>Kondisi Kelas Rusak Berat: {rusakBerat}</span></div>
          <div className={styles.dataRow}><span>Kekurangan RKB: {kurangRkb}</span></div>
        </div>
      </div>

      {/* Sections for Library and Labs */}
      <div className={styles.section}>
      ...
      {/* KOREKSI BAGIAN FURNITUR DAN KOMPUTER */}
      <div className={styles.section}>
